...
**现在的做法：三档降级，SSL 类错误一律重试**

| 档位 | 配置 | 日志里显示为 |
| --- | --- | --- |
| ① | 校验 HTTPS 证书 | `第1/3次, 校验证书` |
| ② | 关掉证书校验 | `第2/3次, 不校验证书` |
| ③ | 关校验 + **强制 HTTP/1.1** + 至少 TLS1.2 | `第3/3次, 不校验+HTTP1.1+TLS1.2` |

第 ③ 档是专门给这类 CDN 准备的：switch-curl 走 mbedTLS，部分 CDN 在协商
ALPN(h2) 或谈 TLS1.3 时会直接把连接打断，libcurl 侧看到的就只有一句 35。
强制 HTTP/1.1 是这类问题最有效的绕法。
...
| 现象 | 原因 / 处理 |
| --- | --- |
| 图片加载不出来 | 看诊断区（底部按钮展开）每张的原因。若是 35/60 类 SSL 错误，v2.7.0 已做三档降级；仍失败请把 `log.txt` 发来，会逐行记录三次尝试 |
...
| **2.7.0** | 三档 SSL 降级（修图片失败）· 文字模式按键修复（去掉阻塞的 `appletMainLoop` + 补 hid 初始化）· 诊断信息改就地展开（修点击报错退出） |
